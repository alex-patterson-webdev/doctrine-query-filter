...
final class SortFactory implements SortFactoryInterface
{
    ...
    public function create(QueryFilterManagerInterface $manager, string $name, array $options = []): SortInterface
    {
        ...
        if (!class_exists($className) || !is_a($className, SortInterface::class, true)) {
        ...
        try {
            ...
            throw new SortFactoryException(sprintf('Failed to create sort filter \'%s\'', $name), $e->getCode(), $e);
        }
    }
}
